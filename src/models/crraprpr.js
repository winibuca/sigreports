...
module.exports = {
  ...
  async getResumenCliente(context) {
    const { p_codvor, p_ofcsol, p_codsol } = context;
    ...
    let query = `
      BEGIN
        :resumenCliente := OPS$COLOCA.pqbd_apex_crraprpr.fbd_col_resumen_cliente(:p_codvor,:p_ofcsol,:p_codsol);
      END;
    `;
    ...
    const bindVars = {
      ...
      p_ofcsol,
      p_codsol,
      resumenCliente: { dir: oracledb.BIND_OUT, type: oracledb.CLOB },
    };
    ...
    let clobData = "";
    return new Promise((resolve, reject) => {
      if (result.outBinds.resumenCliente) {
        const clob = result.outBinds.resumenCliente;
